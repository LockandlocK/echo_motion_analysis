...
Scoring = {'AUC':roc_auc_scorer , 'Accuracy':'accuracy', 'Recall': 'recall', 'F1-Score': 'f1', 'Precision': 'precision'}
...
def CNN_train(X_train, y_train, REFIT):
    ...
    grid_search = GridSearchCV(estimator = model, n_jobs = 30, param_grid = param_grid, scoring = Scoring, refit = REFIT, cv = 5)
...
def SVM_train(X_train, y_train, REFIT):
  ...
  svm_model = GridSearchCV(SVC(kernel='linear'), params_grid, n_jobs = 30, scoring = Scoring, refit = REFIT, cv = 5)
  print('SVM Train')
...
def DT_train(X_train, y_train, REFIT):
  ...
  dt_model = GridSearchCV(DecisionTreeClassifier(), params_grid, n_jobs = 30, scoring = Scoring, refit = REFIT, cv = 5)
  print('DT Train')
...
def RF_train(X_train, y_train, REFIT):
  ...
  RF_model = GridSearchCV(RandomForestClassifier(), params_grid, n_jobs = 30, scoring = Scoring, refit = REFIT, cv = 5)
  print('RF Train')
...
def KNN_train(X_train, y_train, REFIT):
  ...
  knn_model = GridSearchCV(KNeighborsClassifier(), params_grid, n_jobs = 30, scoring = Scoring, refit = REFIT, cv = 5)
  print('KNN Train')
